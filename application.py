...
connection = pymysql.connect(
    host=base64.b64decode(config['DEFAULT']['Host'].encode()),
    user=base64.b64decode(config['DEFAULT']['Username'].encode()),
    password=base64.b64decode(config['DEFAULT']['Password'].encode()),
    charset='utf8mb4',
    cursorclass=pymysql.cursors.DictCursor
)
print("Dose the connection is opened? {0}".format(connection.open))
if not connection.open:
    print("Failed to connect to MySQL Server.")
    sys.exit(1)
connection.select_db("usercount")
...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not all((userName, passWord)):
            ...
            resultCount = self.cursor.execute(sql, (userName, passWord))
            if resultCount == 1:
...
handlers.extend([
    ...
    URLSpec(r"/register", RegisterHandler, dict(mysqldb=connection), name="registerHandler"),
    URLSpec(r"/help", HelpHandler, name="helpHandler"),
    URLSpec(r"/login", LoginHandler, dict(mysqldb=connection), name="loginHandler"),
